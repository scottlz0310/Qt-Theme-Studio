...
class TestCompleteWorkflow:
    ...
    def mock_qt_adapter(self):
        ...
        mock_modules = {"QtWidgets": Mock(), "QtCore": Mock(), "QtGui": Mock()}
    ...
    def test_complete_theme_creation_workflow(
        self, temp_dir, mock_qt_adapter, mock_theme_adapter
    ):
        """完全なテーマ作成ワークフローテスト"""
        # 1. テーマコントローラーの初期化
...
class TestQtFrameworkCompatibility:
    ...
    def test_framework_specific_workflow(self, framework):
        ...
        # フレームワーク固有のモックを作成
        with patch(
            "qt_theme_studio.adapters.qt_adapter.QtAdapter"
        ) as mock_qt_adapter_class:
            ...
            mock_qt_adapter.get_qt_modules.return_value = (
                self._create_framework_modules(framework)
            )
    ...
    def test_framework_fallback_workflow(self):
        ...
        with patch(
            ...
            mock_qt_adapter = Mock()
            mock_qt_adapter.detect_qt_framework.side_effect = Exception(
                "Qtフレームワークが見つかりません"
            )
            mock_qt_adapter.get_qt_modules.side_effect = Exception(
                "Qtフレームワークが見つかりません"
            )
            mock_qt_adapter_class.return_value = mock_qt_adapter
...
@pytest.mark.skipif(
    any(cls is None for cls in [ThemeEditor, PreviewWindow]),
    reason="必要なモジュールが利用できません",
)
class TestIntegrationScenarios:
    ...
    def test_multi_user_workflow(self, tmp_path):
        ...
        assert (
            theme1_content["colors"]["primary"] != theme2_content["colors"]["primary"]
        )
    ...
    def test_large_theme_workflow(self):
        ...
        large_theme = {
            ...
            "fonts": {
                f"font_{i}": {"family": "Font{i}", "size": 12} for i in range(100)
            },
        ...
        # フォントとサイズも実装に合わせて確認
        assert (
            len(new_theme["fonts"]) >= 2
        )  # 基本的なフォント設定が含まれていることを確認
        assert "default" in new_theme["fonts"]
        assert "heading" in new_theme["fonts"]
        assert (
            len(new_theme["sizes"]) >= 4
        )  # 基本的なサイズ設定が含まれていることを確認
