...
from qt_theme_studio.services.import_service import (
    ThemeImportService, ImportError
)
...
class TestThemeImportService(unittest.TestCase):
    ...
    def test_normalize_colors(self):
        ...
        self.assertEqual(normalized['background'], '#ffffff')
        self.assertEqual(normalized['surface'], '#ffffff')
    ...
    def test_extract_colors_from_qss(self):
        ...
        self.assertIn('#ffffff', color_values)
    ...
    def test_is_valid_color(self):
        """色値妥当性テスト"""
        # 有効な色値
        self.assertTrue(self.import_service.is_valid_color('#ffffff'))
        self.assertTrue(self.import_service.is_valid_color('#fff'))
