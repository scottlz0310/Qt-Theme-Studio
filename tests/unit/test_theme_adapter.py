...
class TestThemeAdapter:
    ...
    def test_load_unsupported_format(self):
        ...
        with patch.object(Path, 'exists', return_value=True), \
             patch.object(self.adapter, '_is_initialized', True):
            with pytest.raises(ThemeLoadError, match="サポートされていないテーマファイル形式"):
    ...
    def test_json_decode_error(self):
        ...
        with patch.object(Path, 'exists', return_value=True), \
             ...
             patch.object(self.adapter, '_is_initialized', True):
            with pytest.raises(ThemeLoadError, match="JSONファイルの解析に失敗しました"):
                self.adapter.load_theme(str(test_file))
    ...
    def test_export_unsupported_format(self):
        ...
        with pytest.raises(ThemeExportError, match="サポートされていないエクスポート形式: txt"):
