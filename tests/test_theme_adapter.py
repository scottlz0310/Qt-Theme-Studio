...
class TestThemeAdapterColorValidation:
    ...
    def test_is_valid_color_hex(self):
        """16進数カラーコードの検証テスト"""
        assert self.adapter._is_valid_color("#ffffff") is True
        assert self.adapter._is_valid_color("#000") is True
        assert self.adapter._is_valid_color("#ff0000ff") is True  # RGBA
        assert self.adapter._is_valid_color("#gggggg") is False  # 無効な16進数
        assert self.adapter._is_valid_color("#ff") is False  # 長さが不正
