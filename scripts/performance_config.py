    ...
    def validate_config(self) -> List[str]:
        ...
        for benchmark in config.benchmarks:
            ...
            if benchmark.timeout <= 0:
                errors.append(
                    f"タイムアウトは正の値である必要があります: {benchmark.name}"
                )

            if benchmark.min_iterations <= 0:
                errors.append(
                    f"最小反復回数は正の値である必要があります: {benchmark.name}"
                )
        ...
        def validate_thresholds(thresholds: ThresholdConfig, context: str):
            ...
            if thresholds.high <= thresholds.medium:
                errors.append(
                    f"{context}: high閾値はmedium閾値より大きい必要があります"
                )
            if thresholds.critical <= thresholds.high:
                errors.append(
                    f"{context}: critical閾値はhigh閾値より大きい必要があります"
                )
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if isinstance(value, dict):
                    ...
                    result[key] = [
                        self._convert_to_json(item) if isinstance(item, dict) else item
                        for item in value
                    ]
                else:
                    result[key] = value
            ...
            if key == "default_thresholds" and isinstance(value, dict):
                ...
                for item in value:
                    if isinstance(item, dict):
                        ...
                        if item.get("custom_thresholds"):
                            item["custom_thresholds"] = ThresholdConfig(
...
def main():
    ...
    parser.add_argument("--validate", action="store_true", help="設定の妥当性を検証")
    parser.add_argument("--show-config", action="store_true", help="現在の設定を表示")
    parser.add_argument(
        "--create-default", action="store_true", help="デフォルト設定ファイルを作成"
    )
