...
class IntegratedQualityDashboard:
    ...
    def collect_workflow_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rates = {}
            for workflow_name in available_workflows:
                executions = [e for e in recent_executions if e.get("workflow") == workflow_name]
                if executions:
            ...
            return {
                ...
                "enabled_workflows": sum(1 for config in workflow_configs.values() if config["enabled"])
            }
    ...
    def _get_recent_workflow_executions(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for log_file in log_files:
                try:
                    with open(log_file, 'r', encoding='utf-8') as f:
                        for line in f:
                            if "ワークフロー" in line and ("開始" in line or "完了" in line):
                                # ログ行からワークフロー情報を抽出
            ...
            # 最新20件を返す
            return sorted(executions, key=lambda x: x.get("timestamp", ""), reverse=True)[:20]
    ...
    def collect_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning("psutilが利用できません。システムメトリクスをスキップします")
            return {}
        except Exception as e:
    ...
    def collect_security_metrics(self) -> Dict[str, Any]:
        ...
        try:
            # セキュリティスキャン結果を取得
            security_report_file = self.project_root / "logs" / "bandit-security-report.json"
    ...
    def _calculate_security_score(self, severity_counts: Dict[str, int]) -> float:
        ...
        if total_weighted_issues == 0:
            return 10.0
        elif total_weighted_issues <= 5:
            return 9.0
        elif total_weighted_issues <= 10:
            return 8.0
        elif total_weighted_issues <= 20:
            return 7.0
        elif total_weighted_issues <= 30:
            return 6.0
        else:
        ...
        try:
            ...
            if benchmark_file.exists():
                ...
                if benchmarks:
                    ...
                    # 最も遅いテスト
                    slowest_test = max(benchmarks, key=lambda x: x.get("stats", {}).get("mean", 0))
                    ...
                    return {
                        ...
                        "performance_score": self._calculate_performance_score(overall_avg)
                    }
    ...
    def _check_alerts(self, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for workflow_name, success_rate in success_rates.items():
                if success_rate < 80:
                    self.logger.warning(f"🚨 ワークフロー '{workflow_name}' の成功率が低いです: {success_rate:.1f}%")
    ...
    def generate_integrated_report(self) -> str:
        ...
        report.append("=" * 80)
        report.append(f"生成日時: {datetime.now().strftime('%Y年%m月%d日 %H時%M分%S秒')}")
        report.append("")
        ...
        if workflow_stats:
            ...
            report.append("-" * 40)
            report.append(f"利用可能なワークフロー数: {workflow_stats.get('total_workflows', 0)}")
            report.append(f"有効なワークフロー数: {workflow_stats.get('enabled_workflows', 0)}")
            ...
            if success_rates:
                ...
                for workflow_name, success_rate in success_rates.items():
                    ...
                    report.append(f"  {status_icon} {workflow_name}: {success_rate:.1f}%")
            report.append("")
        ...
        if system_metrics:
            ...
            report.append(f"メモリ使用率: {system_metrics.get('memory_percent', 0):.1f}%")
            report.append(f"ディスク使用率: {system_metrics.get('disk_percent', 0):.1f}%")
            report.append(f"プロセスメモリ: {system_metrics.get('process_memory_mb', 0):.1f}MB")
            report.append("")
        ...
        if security_metrics and "error" not in security_metrics:
            ...
            report.append(f"セキュリティスコア: {security_metrics.get('security_score', 0):.1f}/10")
            report.append("")
        ...
        if performance_metrics and "error" not in performance_metrics:
            ...
            report.append("-" * 40)
            report.append(f"ベンチマーク数: {performance_metrics.get('total_benchmarks', 0)}")
            report.append(f"平均実行時間: {performance_metrics.get('average_execution_time', 0):.3f}秒")
            
            slowest_test = performance_metrics.get("slowest_test", {})
            if slowest_test:
                report.append(f"最遅テスト: {slowest_test.get('name', 'N/A')} ({slowest_test.get('time', 0):.3f}秒)")
            
            report.append(f"パフォーマンス評価: {performance_metrics.get('performance_score', 'N/A')}")
            report.append("")
        ...
        if coverage_data:
            ...
            for module, data in coverage_data.items():
                if module != "TOTAL":
                    coverage_icon = "✅" if data['coverage'] >= 80 else "⚠️" if data['coverage'] >= 60 else "❌"
                    report.append(f"  {coverage_icon} {module}: {data['coverage']:.1f}% ({data['statements']}行)")

            if "TOTAL" in coverage_data:
                total = coverage_data["TOTAL"]
                total_icon = "✅" if total['coverage'] >= 80 else "⚠️" if total['coverage'] >= 60 else "❌"
                report.append(f"  {total_icon} 全体: {total['coverage']:.1f}% ({total['statements']}行)")
            report.append("")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if total_coverage < 50:
            recommendations.append("🚨 カバレッジが50%未満です。テストの追加を最優先で実施してください")
        elif total_coverage < 70:
            recommendations.append("⚠️ カバレッジが70%未満です。テストの追加を検討してください")
        elif total_coverage >= 90:
            recommendations.append("✅ カバレッジは優秀です。現在のレベルを維持してください")
        ...
        for workflow_name, success_rate in success_rates.items():
            if success_rate < 70:
                recommendations.append(f"🔧 ワークフロー '{workflow_name}' の成功率が低いです。設定を見直してください")
        ...
        if security_metrics and "error" not in security_metrics:
            ...
            if severity_counts.get("HIGH", 0) > 0:
                recommendations.append("🚨 高重要度のセキュリティ問題があります。即座に対応してください")
            elif total_issues > 10:
                recommendations.append("⚠️ セキュリティ問題が多数検出されています。定期的な対応を検討してください")
        ...
        if performance_metrics and "error" not in performance_metrics:
            ...
            if avg_time > 2.0:
                recommendations.append("⚡ テストの実行時間が長いです。パフォーマンス最適化を検討してください")
        ...
        if system_metrics:
            ...
            if memory_percent > 80:
                recommendations.append("💾 メモリ使用率が高いです。リソース使用量の最適化を検討してください")
    ...
    def generate_integrated_visualization(self):
        ...
        try:
            ...
            fig.suptitle('Qt-Theme-Studio 統合品質メトリクス', fontsize=16, fontweight='bold')
            ...
            if len(self.metrics_history) > 1:
                ...
                if timestamps:
                    ax6.plot(timestamps, cpu_usage, label="CPU", color="blue", marker='o', markersize=3)
                    ax6.plot(timestamps, memory_usage, label="メモリ", color="red", marker='s', markersize=3)
            ...
            ax7.text(0.5, 0.5, f"品質スコア\n{quality_score}", ha="center", va="center",
                    fontsize=20, fontweight="bold", transform=ax7.transAxes, color=color)
            ...
            # セキュリティスコア
            security_score = security_metrics.get("security_score", 0) if security_metrics else 0
    ...
    def run_integrated_dashboard(self, enable_monitoring: bool = False, monitoring_interval: int = 60):
        """統合ダッシュボードを実行
        ...
        """
        self.logger.info("Qt-Theme-Studio 統合品質メトリクスダッシュボードを開始します...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Qt-Theme-Studio 統合品質メトリクスダッシュボード")
    parser.add_argument("--integrated", action="store_true", help="統合ダッシュボードを実行")
    parser.add_argument("--monitoring", action="store_true", help="リアルタイム監視を有効化")
