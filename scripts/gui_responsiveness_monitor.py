...
class InteractionSimulator:
    ...
    def simulate_text_input(self, widget, text: str) -> ResponseMeasurement:
        ...
        try:
            ...
            return self._create_failed_measurement(
                f"テキスト入力シミュレーションエラー: {e}", event
            )

    def simulate_window_resize(
        self, widget, width: int, height: int
    ) -> ResponseMeasurement:
        """ウィンドウリサイズをシミュレート"""
        if not QT_AVAILABLE or not widget:
            return self._create_failed_measurement("Qt not available or widget is None")
        ...
        try:
            ...
            return self._create_failed_measurement(
                f"ウィンドウリサイズシミュレーションエラー: {e}", event
            )

    def _create_failed_measurement(
        self, error_message: str, event: Optional[InteractionEvent] = None
    ) -> ResponseMeasurement:
        """失敗した測定結果を作成"""
        if event is None:
            event = InteractionEvent(
...
class ResponsivenessMonitor:
    ...
    def run_responsiveness_test(
        self, widget, test_scenarios: List[Dict[str, Any]]
        ...
        for i, scenario in enumerate(test_scenarios):
            self.logger.info(
                f"シナリオ {i + 1}/{len(test_scenarios)} を実行中: {scenario.get('name', 'unnamed')}"
            )
    ...
    def _execute_scenario(
        self, widget, scenario: Dict[str, Any]
    ) -> ResponseMeasurement:
        """個別のテストシナリオを実行"""
    ...
    def _calculate_metrics(
        self, session_id: str, measurements: List[ResponseMeasurement]
    ) -> ResponsivenessMetrics:
        """応答性メトリクスを計算"""
        if not measurements:
        ...
        # 最速・最遅のインタラクション
        slowest_measurement = max(
            successful_measurements, key=lambda m: m.response_time
        )
        ...
        for measurement in measurements:
            ...
            interactions_by_type[event_type] = (
                interactions_by_type.get(event_type, 0) + 1
            )
    ...
    def _save_measurements(self, measurements: List[ResponseMeasurement]) -> None:
        ...
        try:
            ...
            if self.measurements_file.exists():
                with open(self.measurements_file, encoding="utf-8") as f:
                    data = json.load(f)
            ...
            for measurement in measurements:
                ...
                measurement_dict["event"]["timestamp"] = (
                    measurement.event.timestamp.isoformat()
                )
    ...
    def _save_metrics(self, metrics: ResponsivenessMetrics) -> None:
        ...
        try:
            ...
            if metrics.slowest_interaction:
                ...
                slowest_dict["start_time"] = (
                    metrics.slowest_interaction.start_time.isoformat()
                )
                slowest_dict["end_time"] = (
                    metrics.slowest_interaction.end_time.isoformat()
                )
                slowest_dict["event"]["timestamp"] = (
                    metrics.slowest_interaction.event.timestamp.isoformat()
                )
            ...
            if metrics.fastest_interaction:
                ...
                fastest_dict["start_time"] = (
                    metrics.fastest_interaction.start_time.isoformat()
                )
                fastest_dict["end_time"] = (
                    metrics.fastest_interaction.end_time.isoformat()
                )
                fastest_dict["event"]["timestamp"] = (
                    metrics.fastest_interaction.event.timestamp.isoformat()
                )
    ...
    def _analyze_metrics_data(
        self, metrics_data: List[Dict[str, Any]], days: int
    ) -> Dict[str, Any]:
        ...
        total_successful = sum(
            m.get("successful_interactions", 0) for m in metrics_data
        )
        ...
        all_p95_times = [
            m.get("p95_response_time", 0)
            for m in metrics_data
            if m.get("p95_response_time", 0) > 0
        ]
        ...
        for metrics in metrics_data:
            for interaction_type, count in metrics.get(
                "interactions_by_type", {}
            ).items():
                interaction_types[interaction_type] = (
                    interaction_types.get(interaction_type, 0) + count
                )

        # 応答性評価
        responsiveness_grade = self._calculate_responsiveness_grade(
            overall_avg_response_time
        )
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                overall_avg_response_time, success_rate, interaction_types
            ),
        }
    ...
    def _calculate_responsiveness_grade(self, avg_response_time: float) -> str:
        ...
        if avg_response_time <= self.thresholds["excellent"]:
            return "A+ (優秀)"
        if avg_response_time <= self.thresholds["good"]:
            return "A (良好)"
        if avg_response_time <= self.thresholds["acceptable"]:
            return "B (許容範囲)"
    ...
    def _generate_recommendations(
        ...
        if avg_response_time > self.thresholds["poor"]:
            recommendations.append(
                "応答時間が非常に遅いです。UIスレッドでの重い処理を別スレッドに移動することを検討してください。"
            )
        elif avg_response_time > self.thresholds["acceptable"]:
            recommendations.append(
                "応答時間が許容範囲を超えています。処理の最適化を検討してください。"
            )

        # 成功率に基づく提案
        if success_rate < 90:
            recommendations.append(
                "インタラクションの成功率が低いです。エラーハンドリングの改善を検討してください。"
            )
        elif success_rate < 95:
            recommendations.append(
                "インタラクションの成功率を向上させる余地があります。"
            )

        # インタラクションタイプに基づく提案
        if interaction_types.get("window_resize", 0) > 0:
            recommendations.append(
                "ウィンドウリサイズの応答性を改善するため、レイアウトの最適化を検討してください。"
            )

        if interaction_types.get("text_input", 0) > 0:
            recommendations.append(
                "テキスト入力の応答性向上のため、入力検証の最適化を検討してください。"
            )

        if not recommendations:
            recommendations.append(
                "現在の応答性は良好です。この状態を維持してください。"
            )
...
def create_default_test_scenarios() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {"name": "右クリック", "type": "click", "button": "right", "wait_after": 0.1},
        {
            "name": "スペースキー押下",
            "type": "key_press",
...
def main():
    ...
    parser.add_argument(
        "--data-dir", default="logs/performance/gui", help="データ保存ディレクトリ"
    )
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                "logs/gui_responsiveness_monitor.log", encoding="utf-8"
            ),
        ],
    )
    ...
    try:
        if args.test_app:
            if not QT_AVAILABLE:
                print(
                    "❌ Qtが利用できません。PySide6、PyQt6、またはPyQt5をインストールしてください。"
                )
                return 1
            ...
            if metrics.slowest_interaction:
                print(
                    f"最遅インタラクション: {metrics.slowest_interaction.event.event_type} ({metrics.slowest_interaction.response_time:.4f}秒)"
                )

            if metrics.fastest_interaction:
                print(
                    f"最速インタラクション: {metrics.fastest_interaction.event.event_type} ({metrics.fastest_interaction.response_time:.4f}秒)"
                )
