...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = Path(config_path) if config_path else self.project_root / ".kiro" / "workflow" / "config.yml"
        ...
        return {
            "workflows": {
                "pre_commit": {
                    "enabled": True,
                    "steps": [
                        {"name": "ruff_lint", "command": "ruff check .", "required": True},
                        {"name": "ruff_format", "command": "ruff format --check .", "required": True}
                    ]
                }
            },
            "quality_thresholds": {
        ...
        for env_var, config_path in self.env_mappings.items():
            ...
            if env_value is not None:
                try:
                    ...
                    # 変更を記録
                    self._record_change(config_path, old_value, converted_value, "environment", f"環境変数 {env_var}")
                    
                    self.logger.info(f"環境変数 {env_var} で設定を上書き: {config_path} = {converted_value}")
    ...
    def _record_change(self, path: str, old_value: Any, new_value: Any, source: str, reason: str) -> None:
        """設定変更を記録"""
        change = ConfigChange(
    ...
    def validate_config(self) -> ValidationResult:
        ...
        try:
            ...
            if "quality_thresholds" in self.config:
                self._validate_quality_thresholds(self.config["quality_thresholds"], result)
    ...
    def _validate_workflows(self, workflows: Dict[str, Any], result: ValidationResult) -> None:
        """ワークフロー設定を検証"""
        for workflow_name, workflow_config in workflows.items():
            if not isinstance(workflow_config, dict):
                result.errors.append(f"ワークフロー '{workflow_name}' の設定が辞書ではありません")
                continue
            
            # 必須フィールドの確認
            if "steps" not in workflow_config:
                result.errors.append(f"ワークフロー '{workflow_name}' にstepsが定義されていません")
                continue
            
            # ステップの検証
            steps = workflow_config["steps"]
            if not isinstance(steps, list):
                result.errors.append(f"ワークフロー '{workflow_name}' のstepsがリストではありません")
                continue
            
            for i, step in enumerate(steps):
                if not isinstance(step, dict):
                    result.errors.append(f"ワークフロー '{workflow_name}' のステップ {i} が辞書ではありません")
                    continue
                
                if "name" not in step:
                    result.errors.append(f"ワークフロー '{workflow_name}' のステップ {i} にnameが定義されていません")
                
                if "command" not in step and "plugin" not in step:
                    result.warnings.append(f"ワークフロー '{workflow_name}' のステップ '{step.get('name', i)}' にcommandまたはpluginが定義されていません")

    def _validate_quality_thresholds(self, thresholds: Dict[str, Any], result: ValidationResult) -> None:
        """品質閾値設定を検証"""
        # カバレッジ閾値
        if "coverage_minimum" in thresholds:
            ...
            if not isinstance(coverage, (int, float)) or not 0 <= coverage <= 100:
                result.errors.append("coverage_minimumは0-100の数値である必要があります")
        ...
        if "test_success_rate" in thresholds:
            ...
            if not isinstance(success_rate, (int, float)) or not 0 <= success_rate <= 100:
                result.errors.append("test_success_rateは0-100の数値である必要があります")
        
        # セキュリティスコア
        if "security_score" in thresholds:
            security_score = thresholds["security_score"]
            if not isinstance(security_score, (int, float)) or not 0 <= security_score <= 10:
                result.errors.append("security_scoreは0-10の数値である必要があります")

    def _validate_notifications(self, notifications: Dict[str, Any], result: ValidationResult) -> None:
        """通知設定を検証"""
        if "channels" in notifications:
            channels = notifications["channels"]
            if not isinstance(channels, list):
                result.errors.append("notification channelsはリストである必要があります")
                return
            ...
            for i, channel in enumerate(channels):
                ...
                if "type" not in channel:
                    result.errors.append(f"通知チャンネル {i} にtypeが定義されていません")
                
                channel_type = channel.get("type")
                if channel_type not in ["console", "file", "email"]:
                    result.warnings.append(f"未知の通知チャンネルタイプ: {channel_type}")

    def _validate_environments(self, environments: Dict[str, Any], result: ValidationResult) -> None:
        """環境設定を検証"""
    ...
    def _auto_fix_config(self) -> bool:
        ...
        try:
            ...
            if success_rate is not None and (not isinstance(success_rate, (int, float)) or not 0 <= success_rate <= 100):
        ...
        return {
            ...
            "properties": {
                ...
                "quality_thresholds": {
                    "type": "object",
                    "properties": {
                        "coverage_minimum": {"type": "number", "minimum": 0, "maximum": 100},
                        "test_success_rate": {"type": "number", "minimum": 0, "maximum": 100}
                    }
                }
            }
        }
...
def main():
    ...
    try:
        ...
        # 設定を表示
        print("現在の設定:")
        print(yaml.dump(config_manager.get_config(), default_flow_style=False, allow_unicode=True))
