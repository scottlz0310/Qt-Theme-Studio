...
class DocumentationGapAnalyzer:
    ...
    def _identify_gaps(self, code_analysis: Dict[str, Any], related_docs: List[Path], mention_check: Dict[str, Any]) -> List[Dict[str, Any]]:
        """ドキュメントギャップを特定
        ...
        # docstringの不足
        functions_without_docstring = [f for f in public_functions if not f["has_docstring"]]
        classes_without_docstring = [c for c in public_classes if not c["has_docstring"]]
        ...
        # ドキュメント内での言及不足
        if mention_check["total_mentions"] == 0 and (public_functions or public_classes):
            gaps.append({
...
class DocumentationUpdateChecker:
    ...
    def check_recent_changes(self, days: int = 7) -> Dict[str, Any]:
        ...
        for py_file_path in changed_python_files:
            ...
            if py_file.exists():
                # 関連ドキュメントを検索
                related_docs = self.gap_analyzer.doc_mapper.find_related_documentation(py_file)
                ...
                if related_docs and not updated_related_docs:
                    update_recommendations.append({
                        ...
                        "related_docs": [str(d.relative_to(self.project_root)) for d in related_docs],
    ...
    def generate_update_report(self, check_result: Dict[str, Any], suggestions: Dict[str, Any]) -> str:
        """ドキュメント更新レポートを生成
        ...
        # 高優先度の提案を詳細表示
        high_priority_suggestions = [s for s in suggestions["suggestions"] if s["priority"] == "high"]
        if high_priority_suggestions:
            ...
            for suggestion in high_priority_suggestions:
                ...
                if "suggested_path" in suggestion:
                    report_lines.append(f"**推奨作成先**: {suggestion['suggested_path']}")
                report_lines.append("")
