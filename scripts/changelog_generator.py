...
class ChangelogGenerator:
    ...
    def get_git_commits(
        self, since_tag: Optional[str] = None, until_tag: Optional[str] = None
    ) -> List[Dict]:
        """Gitコミット情報を取得"""
    ...
    def classify_commit(self, commit: Dict) -> Tuple[str, str]:
        ...
        if any(keyword in subject for keyword in ["feat", "add", "新機能", "追加"]):
            return "追加", self._clean_commit_message(commit["subject"])
        if any(keyword in subject for keyword in ["remove", "delete", "削除"]):
            return "削除", self._clean_commit_message(commit["subject"])
        if any(keyword in subject for keyword in ["security", "セキュリティ"]):
    ...
    def generate_changelog_section(
        self, commits: List[Dict], version: str = "未リリース"
    ) -> str:
        """変更履歴セクションを生成"""
    ...
    def update_changelog_file(
        self, new_section: str, version: str = "未リリース"
    ) -> bool:
        """CHANGELOG.mdファイルを更新"""
...
使用例:
  ...
  python scripts/changelog_generator.py --output changelog_preview.md
    ...
    parser.add_argument("--since", help="開始タグ（このタグ以降のコミットを含める）")
    parser.add_argument("--until", help="終了タグ（このタグまでのコミットを含める）")
    parser.add_argument(
        "--version",
        default="未リリース",
    ...
    parser.add_argument(
        "--output", help="出力ファイル（指定しない場合はCHANGELOG.mdを更新）"
    )
    parser.add_argument(
        "--preview", action="store_true", help="プレビューのみ（ファイルを更新しない）"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.preview:
            # プレビューのみ
            changelog = generator.preview_changelog(
                args.since, args.until, args.version
            )
            print(changelog)
        else:
            # 変更履歴を生成
