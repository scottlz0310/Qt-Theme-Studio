...
class MemoryProfiler:
    ...
    def take_snapshot(self, context: str = "") -> MemorySnapshot:
        ...
        try:
            ...
            logger.debug(
                f"メモリスナップショットを取得: {process_memory_mb:.1f}MB ({context})"
            )
            return snapshot
    ...
    def detect_memory_leaks(self, duration_minutes: int = 5) -> List[MemoryLeak]:
        ...
        for i in range(len(recent_snapshots) - window_size):
            ...
            # 平均メモリ使用量を計算
            start_avg_memory = sum(s.process_memory_mb for s in start_window) / len(
                start_window
            )
            end_avg_memory = sum(s.process_memory_mb for s in end_window) / len(
                end_window
            )

            # 時間差を計算
            time_diff = (
                end_window[-1].timestamp - start_window[0].timestamp
            ).total_seconds()
    ...
    def profile_function(
        ...
        try:
            ...
            tracemalloc_diff = (
                after_snapshot.tracemalloc_current_mb
                - before_snapshot.tracemalloc_current_mb
            )
    ...
    def generate_memory_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        report = {
            ...
            "system_memory": {
                "current_percent": system_memory_values[-1]
            ...
            "tracemalloc": {
                ...
                "peak_mb": max(s.tracemalloc_peak_mb for s in recent_snapshots)
                if recent_snapshots
                else 0,
            },
            "gc_objects": {
                ...
                "avg_count": sum(gc_objects_values) / len(gc_objects_values)
                if gc_objects_values
                else 0,
            },
            "memory_leaks": {
        ...
        report_file = (
            self.reports_dir
            / f"memory_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _determine_leak_severity(self, leak_rate: float, total_leaked: float) -> str:
        ...
        if leak_rate > 10.0 or total_leaked > 100.0:
            return "CRITICAL"
        if leak_rate > 5.0 or total_leaked > 50.0:
            return "HIGH"
        if leak_rate > 2.0 or total_leaked > 20.0:
            return "MEDIUM"
        return "LOW"
    ...
    def _analyze_memory_leak(
        ...
        if gc_diff > 0:
            ...
            analysis_parts.append(
                f"GCオブジェクト数が{gc_diff}個増加（{gc_rate:.1f}個/秒）"
            )
    ...
    def _check_threshold_violations(
        ...
        """閾値違反をチェック"""
        violations = {"process_memory": 0, "system_memory": 0, "total": 0}
    ...
    def _generate_recommendations(
        self, snapshots: List[MemorySnapshot], leaks: List[MemoryLeak]
    ) -> List[str]:
        """推奨事項を生成"""
        recommendations = []
    ...
    def _save_leaks(self, leaks: List[MemoryLeak]) -> None:
        ...
        try:
            ...
            existing_leaks = [
                l for l in existing_leaks if l.start_snapshot.timestamp >= cutoff_time
            ]
    ...
    def _load_leaks(self) -> List[MemoryLeak]:
        ...
        try:
            with open(self.leaks_file, encoding="utf-8") as f:
                data = json.load(f)
            ...
            for item in data:
                ...
                start_dict["timestamp"] = datetime.fromisoformat(
                    start_dict["timestamp"]
                )
...
def main():
    ...
    parser.add_argument(
        "--monitor", type=int, metavar="SECONDS", help="指定秒数間メモリ監視を実行"
    )
    parser.add_argument(
        "--detect-leaks",
        type=int,
    ...
    parser.add_argument(
        "--snapshot", action="store_true", help="現在のメモリスナップショットを取得"
    )
    parser.add_argument(
        "--data-dir", default="logs/performance/memory", help="データ保存ディレクトリ"
    )
    ...
    try:
        if args.snapshot:
            ...
            print(
                f"  tracemalloc: {snapshot.tracemalloc_current_mb:.1f}MB (ピーク: {snapshot.tracemalloc_peak_mb:.1f}MB)"
            )
            ...
            print(f"GCオブジェクト数: {snapshot.gc_objects_count:,}個")
            print("\n使用方法:")
            print("  --monitor SECONDS     : メモリ監視")
            print("  --detect-leaks MINUTES: リーク検出")
            print("  --generate-report HOURS: レポート生成")
            print("  --snapshot            : スナップショット取得")
