...
class DependencyChecker:
    ...
    def check_qt_theme_manager(self) -> bool:
        ...
        try:
            ...
            if not latest_version:
                self.logger.info("ℹ️ GitHub APIが利用できないため、現在のバージョンを最新として扱います")
            ...
            if comparison == "outdated":
                self.logger.warning(f"⚠️ 更新が利用可能です: {current_version} -> {latest_version}")
        ...
        try:
            ...
            try:
                ...
                if outdated_packages:
                    self.logger.warning(f"⚠️ {len(outdated_packages)}個のパッケージに更新が利用可能です")
                    
                    # 重要なパッケージの更新を推奨事項に追加
                    important_packages = ["pytest", "ruff", "black", "mypy", "safety", "bandit"]
                    ...
                    for pkg in outdated_packages:
                        ...
                        # 最初の5個のみログ出力
                        if len([p for p in outdated_packages if outdated_packages.index(p) < 5]) > outdated_packages.index(pkg):
                            self.logger.info(f"  - {pkg_name}: {current_ver} -> {latest_ver}")
    ...
    def generate_update_report(self) -> Dict:
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"  - 更新可能: {'はい' if qtm['update_available'] else 'いいえ'}\n")
