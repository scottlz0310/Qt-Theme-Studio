...
class WorkflowEngine:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = config_path or self.project_root / ".kiro" / "workflow" / "config.yml"
    ...
    def _load_configuration(self) -> None:
        ...
        try:
            if Path(self.config_path).exists():
                ...
                self.logger.warning(f"設定ファイルが見つかりません。デフォルト設定を使用します: {self.config_path}")
            ...
            "workflows": {
                "pre_commit": {
                    ...
                    "steps": [
                        ...
                        {"name": "basic_tests", "command": "pytest tests/unit/ -x", "required": False}
                    ]
                },
                "ci_pipeline": {
                    "enabled": True,
                    "steps": [
                        {"name": "quality_check", "command": "python scripts/quality_check.py", "required": True},
                        {"name": "full_tests", "command": "pytest", "required": True},
                        {"name": "security_scan", "command": "python scripts/security_scanner.py", "required": False}
                    ]
                },
                "release": {
                    "enabled": True,
                    "steps": [
                        ...
                        {"name": "build_packages", "command": "python -m build", "required": True},
                        {"name": "generate_checksums", "command": "python scripts/build_release.py --checksums", "required": True}
                    ]
                }
            },
            "quality_thresholds": {
    ...
    def _apply_environment_overrides(self) -> None:
        ...
        if coverage_min := os.getenv("WORKFLOW_COVERAGE_MIN"):
            try:
                self.config.setdefault("quality_thresholds", {})["coverage_minimum"] = float(coverage_min)
    ...
    def validate_environment(self) -> bool:
        ...
        try:
            ...
            if sys.version_info < (3, 8):
                self.logger.error(f"Python 3.8以上が必要です。現在のバージョン: {sys.version}")
                return False
    ...
    async def execute_pipeline(self, pipeline_name: str, **kwargs) -> WorkflowResult:
        ...
        try:
            ...
            if not pipeline_config:
                raise ConfigurationError(f"パイプライン '{pipeline_name}' の設定が見つかりません")
            ...
            for step_config in steps:
                ...
                # 必須ステップが失敗した場合は中断
                if step_result.status == StepStatus.FAILURE and step_config.get("required", True):
                    self.logger.error(f"必須ステップ '{step_result.name}' が失敗しました。パイプラインを中断します")
                    result.status = WorkflowStatus.FAILURE
                    break
            
            # 最終ステータスを決定
            if result.status == WorkflowStatus.RUNNING:
                failed_steps = [s for s in result.steps if s.status == StepStatus.FAILURE]
                if failed_steps:
            ...
            self.logger.error(f"パイプライン '{pipeline_name}' の実行中にエラーが発生しました: {e}")
        ...
        try:
            ...
            if command:
                ...
                if process.returncode == 0:
                    ...
                    result.error = f"コマンドが終了コード {process.returncode} で失敗しました"
                    self.logger.error(f"ステップ '{step_name}' が失敗しました: {result.error}")
            else:
                # カスタムステップ実行（プラグイン対応）
                plugin_name = step_config.get("plugin")
                if plugin_name and plugin_name in self.plugins:
                    plugin_result = await self.plugins[plugin_name].execute(step_config, **kwargs)
                    result.status = StepStatus.SUCCESS if plugin_result else StepStatus.FAILURE
                else:
                    result.status = StepStatus.SKIPPED
                    self.logger.warning(f"ステップ '{step_name}' をスキップしました（コマンドまたはプラグインが指定されていません）")
            
        except Exception as e:
            result.status = StepStatus.FAILURE
            result.error = str(e)
            self.logger.error(f"ステップ '{step_name}' の実行中にエラーが発生しました: {e}")
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if workflow_name in workflows:
            ...
            return 0 if result.status in [WorkflowStatus.SUCCESS, WorkflowStatus.WARNING] else 1
        else:
