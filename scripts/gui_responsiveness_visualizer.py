...
class ResponsivenessVisualizer:
    ...
    def create_response_time_trend_chart(self, days: int = 30) -> Optional[str]:
        ...
        try:
            ...
            # 応答時間の線グラフ
            ax.plot(
                timestamps, avg_times, label="平均応答時間", linewidth=2, marker="o"
            )
            ax.plot(
                timestamps,
                p95_times,
    ...
    def create_interaction_type_distribution_chart(
        self, days: int = 30
    ) -> Optional[str]:
        """
    ...
    def create_success_rate_chart(self, days: int = 30) -> Optional[str]:
        ...
        try:
            ...
            ax1.axhline(
                y=95, color="orange", linestyle="--", alpha=0.7, label="目標値 (95%)"
            )
            ax1.axhline(
                y=90, color="red", linestyle="--", alpha=0.7, label="最低値 (90%)"
            )
            ax1.set_ylabel("成功率 (%)")
            ...
            for ax in [ax1, ax2]:
                ...
                ax.xaxis.set_major_locator(
                    mdates.DayLocator(interval=max(1, days // 10))
                )
    ...
    def create_comprehensive_dashboard(self, days: int = 30) -> Optional[str]:
        ...
        try:
            ...
            # 全体タイトル
            fig.suptitle(
                f"GUI応答性ダッシュボード ({days}日間)", fontsize=16, fontweight="bold"
            )
    ...
    def _plot_statistics_summary(self, ax, metrics_data: List[Dict[str, Any]]):
        ...
        # 全期間の統計を計算
        all_avg_times = [
            d.get("average_response_time", 0)
            for d in metrics_data
            if d.get("average_response_time", 0) > 0
        ]
        total_interactions = sum(d.get("total_interactions", 0) for d in metrics_data)
        total_successful = sum(
            d.get("successful_interactions", 0) for d in metrics_data
        )
...
セッション数: {len(metrics_data)}"""
    ...
    def _generate_html_template(
        ...
        trend_chart: Optional[str],
        distribution_chart: Optional[str],
        success_chart: Optional[str],
        heatmap_chart: Optional[str],
        dashboard_chart: Optional[str],
    ) -> str:
        """HTMLテンプレートを生成"""
...
def main():
    ...
    parser.add_argument(
        "--create-charts", action="store_true", help="すべてのチャートを作成"
    )
    parser.add_argument(
        "--create-dashboard", action="store_true", help="ダッシュボードを作成"
    )
    parser.add_argument(
        "--create-html-report", action="store_true", help="HTMLレポートを作成"
    )
    parser.add_argument("--days", type=int, default=30, help="対象期間（日数）")
    parser.add_argument(
        "--data-dir", default="logs/performance/gui", help="データディレクトリ"
    )
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                "logs/gui_responsiveness_visualizer.log", encoding="utf-8"
            ),
        ],
    )
    ...
    if not MATPLOTLIB_AVAILABLE:
        print(
            "❌ matplotlibが利用できません。pip install matplotlibでインストールしてください。"
        )
        return 1
    ...
    try:
        if args.create_charts:
            ...
            distribution_chart = visualizer.create_interaction_type_distribution_chart(
                args.days
            )
