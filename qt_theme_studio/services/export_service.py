...
class CompatibilityReport:
    ...
    def add_framework_result(
        self, framework: str, is_compatible: bool, details: Dict[str, Any] = None
    ) -> None:
        """フレームワークの結果を追加する
    ...
    def __str__(self) -> str:
        ...
        result = "互換性レポート\n"
        result += (
            "全体的な互換性: {'互換' if self.overall_compatibility else '非互換'}\n\n"
        )

        if self.framework_results:
            result += "フレームワーク別結果:\n"
            for framework, data in self.framework_results.items():
                "互換" if data["compatible"] else "非互換"
...
class ExportService:
    ...
    def export_theme(
        ...
        """
        self.logger.debug("テーマを{format_type}形式でエクスポートします")

        try:
            format_type = format_type.lower()

            if format_type not in self.supported_formats:
                raise ExportError("サポートされていない形式です: {format_type}")
            ...
            if format_type == "json":
                ...
                raise ExportError("未実装の形式です: {format_type}")
            ...
            if output_path:
                ...
                if success:
                    self.logger.info("テーマを{safe_path}にエクスポートしました")
                else:
                    raise ExportError("ファイルの書き込みに失敗しました: {safe_path}")
    ...
    def _export_to_qss(self, theme_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # ヘッダー情報
            theme_data.get("name", "Unnamed Theme")
            theme_data.get("version", "1.0.0")
            qss_lines.append("/* {theme_name} */")
            qss_lines.append("/* Version: {theme_version} */")
            qss_lines.append("/* Generated by Qt-Theme-Studio */")
            qss_lines.append("")
    ...
    def _add_qss_widget_styles(
        ...
        if colors.get("background") or colors.get("text"):
            ...
            if isinstance(default_font, dict):
                if default_font.get("family"):
                    qss_lines.append("    font-family: {default_font['family']};")
                if default_font.get("size"):
                    qss_lines.append("    font-size: {default_font['size']}pt;")
        ...
        if colors.get("primary"):
            ...
            sizes.get("padding", 8)
            sizes.get("border_radius", 4)
            qss_lines.append("    padding: {padding}px;")
            qss_lines.append("    border-radius: {border_radius}px;")
            qss_lines.append("}")
            qss_lines.append("")

            # ホバー状態
            qss_lines.append("QPushButton:hover {")
            colors.get("secondary", colors["primary"])
            qss_lines.append("    background-color: {hover_color};")
            qss_lines.append("}")
            qss_lines.append("")

            # 押下状態
            qss_lines.append("QPushButton:pressed {")
            qss_lines.append("    background-color: {colors.get('primary')};")
        ...
        if colors.get("surface") or colors.get("text"):
            ...
            if colors.get("surface"):
                qss_lines.append("    background-color: {colors['surface']};")
            if colors.get("text"):
                qss_lines.append("    color: {colors['text']};")

            border_color = colors.get("border", "#cccccc")
            sizes.get("border_width", 1)
            sizes.get("border_radius", 4)
            sizes.get("padding", 8)

            qss_lines.append("    border: {border_width}px solid {border_color};")
            qss_lines.append("    border-radius: {border_radius}px;")
            qss_lines.append("    padding: {padding}px;")
            qss_lines.append("}")
            qss_lines.append("")
        ...
        if colors.get("surface"):
            ...
            qss_lines.append("    background-color: {colors['surface']};")
            qss_lines.append("    color: {colors.get('text', '#000000')};")
    ...
    def _export_to_css(self, theme_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            theme_data.get("name", "Unnamed Theme")
            theme_data.get("version", "1.0.0")
            css_lines.append("/* {theme_name} */")
            css_lines.append("/* Version: {theme_version} */")
            css_lines.append("/* Generated by Qt-Theme-Studio */")
            css_lines.append("")
            ...
            for size_name, size_value in sizes.items():
                css_lines.append("    --size-{size_name}: {size_value}px;")
    ...
    def _add_css_base_styles(
        ...
        if isinstance(default_font, dict):
            if default_font.get("family"):
                css_lines.append("    font-family: {default_font['family']};")
            if default_font.get("size"):
                css_lines.append("    font-size: {default_font['size']}pt;")
    ...
    def _export_to_yaml(self, theme_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            return yaml.dump(
                theme_data, default_flow_style=False, allow_unicode=True, indent=2
            )
        except ImportError:
        ...
        try:
            ...
            for framework in frameworks:
                try:
                    ...
                    # テーマの適用テスト
                    compatibility_result = self._test_theme_application(
                    ...
                    self.logger.warning(
                        f"{framework}の互換性テスト中にエラー: {str(e)}"
                    )
                    report.add_framework_result(
                        framework,
                        False,
                        {"reason": "テスト中にエラーが発生しました: {str(e)}"},
                    )
            ...
            self.logger.debug(
                "互換性テスト完了: {'互換' if report.overall_compatibility else '非互換'}"
            )
            return report
    ...
    def _test_theme_application(
        self, theme_data: Dict[str, Any], framework: str
    ) -> Dict[str, Any]:
        """テーマの適用テストを実行する"""
        result = {"compatible": True, "tested_features": [], "issues": []}
    ...
    def export_preview_image(
        self, preview_widget, output_path: Union[str, Path], format_type: str = "png"
    ) -> bool:
        """プレビュー画像をエクスポートする
        ...
        Raises:
            ExportError: エクスポートに失敗した場合
        """
        self.logger.debug("プレビュー画像を{format_type}形式でエクスポートします")
        ...
        try:
            ...
            if hasattr(preview_widget, "grab"):
                ...
                if success:
                    self.logger.info("プレビュー画像を{safe_path}に保存しました")
                    return True
                else:
                    raise ExportError("画像の保存に失敗しました")
            else:
                raise ExportError(
                    "プレビューウィジェットが画像キャプチャに対応していません"
                )

        except Exception as e:
            self.logger.error(
                "プレビュー画像エクスポート中にエラーが発生しました: {str(e)}"
            )
            if isinstance(e, ExportError):
                raise
            raise ExportError(
    ...
    def validate_export_data(self, theme_data: Dict[str, Any]) -> List[str]:
        ...
        for field in required_fields:
            if field not in theme_data:
                errors.append("必須フィールドが不足しています: {field}")
