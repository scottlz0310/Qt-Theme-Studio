...
class ThemeImportService:
    ...
    def import_theme(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not file_path.exists():
                raise ImportError("ファイルが見つかりません: {file_path}")

            file_extension = file_path.suffix.lower()

            if file_extension not in self.supported_formats:
                raise ImportError(
                    "サポートされていないファイル形式です: {file_extension}\n"
    ...
    def normalize_json_theme(self, data: Any) -> Dict[str, Any]:
        ...
        normalized = {
            ...
            "display_name": data.get("display_name", data.get("name", "未命名テーマ")),
        ...
        if "color_scheme" in data:
            normalized["colors"] = self.normalize_color_scheme(data["color_scheme"])
        elif "colors" in data:
        ...
        if "accessibility_features" in data:
            normalized["accessibility_features"] = data["accessibility_features"]

        # パフォーマンス設定
        if "performance_settings" in data:
            normalized["performance_settings"] = data["performance_settings"]
        ...
        if "custom_properties" in data:
            normalized["custom_properties"] = data["custom_properties"]
    ...
    def normalize_color_scheme(self, color_scheme: Dict[str, Any]) -> Dict[str, str]:
        """
        color_scheme形式の色情報を正規化
    ...
    def parse_qss_content(self, qss_content: str, theme_name: str) -> Dict[str, Any]:
        ...
        theme_data = {
            "name": theme_name,
            "version": "1.0.0",
            "description": "{theme_name} (QSSからインポート)",
            "colors": {},
            "fonts": {},
            "sizes": {},
    ...
    def parse_css_content(self, css_content: str, theme_name: str) -> Dict[str, Any]:
        ...
        theme_data = {
            "name": theme_name,
            "version": "1.0.0",
            "description": "{theme_name} (CSSからインポート)",
            "colors": {},
            "fonts": {},
            "sizes": {},
    ...
    def extract_colors_from_css(self, css_content: str) -> Dict[str, str]:
        ...
        # CSS変数の抽出
        css_vars = re.findall(
            r"--[\w-]+\s*:\s*(#[0-9a-fA-F]{6}|#[0-9a-fA-F]{3}|\w+)", css_content
        )

        colors = {}
        for i, color_value in enumerate(css_vars):
            color_name = "css_var_{i}"
    ...
    def add_import_metadata(
        self, theme_data: Dict[str, Any], file_path: Path
    ) -> Dict[str, Any]:
        """
        インポートメタデータの追加
    ...
    def validate_imported_theme(self, theme_data: Dict[str, Any]) -> List[str]:
        ...
        for field in required_fields:
            if field not in theme_data or not theme_data[field]:
                errors.append("必須フィールド '{field}' が不足しています")
