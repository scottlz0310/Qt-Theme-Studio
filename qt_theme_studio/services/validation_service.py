...
class AccessibilityReport:
    ...
    def __str__(self) -> str:
        """文字列表現を返す"""
        result = "アクセシビリティレポート (WCAG {self.wcag_level})\n"
        result += (
            "スコア: {self.score:.1f}% ({self.passed_checks}/{self.total_checks})\n"
        )
        result += "準拠状況: {'準拠' if self.is_compliant() else '非準拠'}\n\n"

        if self.contrast_ratios:
            result += "コントラスト比:\n"
            for pair, ratio in self.contrast_ratios.items():
                result += "  {pair}: {ratio:.2f}\n"
            result += "\n"

        if self.violations:
            result += "違反 ({len(self.violations)}):\n"
        ...
        if self.suggestions:
            result += "改善提案 ({len(self.suggestions)}):\n"
...
class ValidationService:
        ...
        try:
            ...
            for field in required_fields:
                if field not in theme_data:
                    ...
                    errors.append("必須フィールドがNullです: {field}")
            ...
            self.logger.debug("テーマ構造検証完了: {len(errors)}個のエラー")
            return errors
    ...
    def _validate_colors_structure(self, colors: Dict[str, Any]) -> List[str]:
        ...
        for color_name in essential_colors:
            if color_name not in colors:
                errors.append("必須の色が不足しています: {color_name}")

        # 色値の形式検証
        for color_name, color_value in colors.items():
            if not self._is_valid_color_value(color_value):
                errors.append("無効な色値です: {color_name} = {color_value}")
        ...
        for font_name, font_config in fonts.items():
            if isinstance(font_config, dict):
                if "family" not in font_config:
                    errors.append("フォント設定にfamilyが不足しています: {font_name}")
                elif not isinstance(font_config["family"], str):
                    errors.append(
                        "フォントfamilyは文字列である必要があります: {font_name}"
                    )

                if "size" in font_config:
                    if not isinstance(font_config["size"], (int, float)):
                        errors.append(
                            "フォントサイズは数値である必要があります: {font_name}"
                        )
                    elif font_config["size"] <= 0:
                        errors.append(
                            "フォントサイズは正の値である必要があります: {font_name}"
                        )
                ...
                errors.append(
                    "フォント設定は辞書または文字列である必要があります: {font_name}"
                )
    ...
    def _validate_sizes_structure(self, sizes: Dict[str, Any]) -> List[str]:
        ...
        for size_name, size_value in sizes.items():
            if not isinstance(size_value, (int, float)):
                errors.append("サイズ値は数値である必要があります: {size_name}")
            elif size_value < 0:
                errors.append("サイズ値は非負の値である必要があります: {size_name}")
    ...
    def _is_valid_color_value(self, color_value: Any) -> bool:
        ...
        if re.match(r"^rgb\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\)$", color_value):
            return True
        if re.match(
            r"^rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*[\d.]+\s*\)$", color_value
        ):
            return True
        ...
        if re.match(r"^hsl\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*\)$", color_value):
            return True
        if re.match(
            r"^hsla\(\s*\d+\s*,\s*\d+%\s*,\s*\d+%\s*,\s*[\d.]+\s*\)$", color_value
        ):
            return True
    ...
    def validate_wcag_compliance(
        self, theme_data: Dict[str, Any], wcag_level: str = "AA"
    ) -> AccessibilityReport:
        """WCAG準拠を検証する
        ...
        """
        self.logger.debug("WCAG {wcag_level} 準拠検証を開始します")
        ...
        try:
            ...
            self.logger.debug("WCAG検証完了: スコア {report.score:.1f}%")
            return report
    ...
    def _validate_contrast_ratios(
        self, colors: Dict[str, str], report: AccessibilityReport
    ) -> None:
        """コントラスト比を検証する"""
        # 重要な色の組み合わせを定義
        color_combinations = [
                ...
                try:
                    ...
                    if ratio >= min_ratio:
                        ...
                        report.add_violation(
                            ...
                            "{pair_name}のコントラスト比が不十分です: {ratio:.2f} (最小: {min_ratio})",
                            "error",
                            "{pair_name}の色を調整してコントラスト比を{min_ratio}以上にしてください",
                        )
            ...
            try:
                ...
                if ratio >= 1.5:
                    ...
                    report.add_violation(
                        ...
                        "{color1_name}と{color2_name}の色が似すぎています: {ratio:.2f}",
                        "warning",
                        "{color1_name}と{color2_name}をより区別しやすい色に変更してください",
                    )
    ...
    def _validate_font_accessibility(
        self, fonts: Dict[str, Any], report: AccessibilityReport
    ) -> None:
        """フォントのアクセシビリティを検証する"""
        if not fonts:
            report.add_violation(
