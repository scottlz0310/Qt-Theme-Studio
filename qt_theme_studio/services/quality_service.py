...
from .validation_service import ValidationService, AccessibilityReport
...
class QualityReport:
    ...
    def add_quality_check(self, check_name: str, passed: bool, 
                         score: float, details: str = None) -> None:
        """品質チェック結果を追加する
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'accessibility_report': self.accessibility_report.to_dict() if self.accessibility_report else None,
    ...
    def __str__(self) -> str:
        """文字列表現を返す"""
        result = f"品質レポート: {self.theme_name}\n"
        result += f"生成日時: {self.timestamp.strftime('%Y-%m-%d %H:%M:%S')}\n"
        result += f"総合スコア: {self.overall_score:.1f}/100\n\n"
        
        if self.structure_errors:
            result += f"構造エラー ({len(self.structure_errors)}):\n"
        ...
        if self.quality_checks:
            ...
            for check_name, check_data in self.quality_checks.items():
                ...
                result += f"  {status} {check_name}: {check_data['score']:.1f}/100\n"
                if check_data['details']:
                    result += f"    {check_data['details']}\n"
            result += "\n"
        
        if self.performance_metrics:
            result += "パフォーマンス指標:\n"
            for metric, value in self.performance_metrics.items():
                result += f"  {metric}: {value:.3f}秒\n"
            result += "\n"
        ...
        if self.recommendations:
            result += f"推奨事項 ({len(self.recommendations)}):\n"
...
class QualityService:
    ...
    def run_comprehensive_quality_check(self, theme_data: Dict[str, Any], 
                                      theme_name: str = None) -> QualityReport:
        """包括的な品質チェックを実行する
        ...
        self.logger.info(f"包括的品質チェックを開始します: {theme_name}")
        ...
        try:
            ...
            self.logger.info(f"品質チェック完了: {theme_name} (スコア: {report.overall_score:.1f})")
            return report
            
        except Exception as e:
            self.logger.error(f"品質チェック中にエラーが発生しました: {str(e)}")
            report.add_quality_check(
                "error_handling",
                False,
                0.0,
                f"品質チェック中にエラーが発生しました: {str(e)}"
            )
            return report
    
    def _check_structure_quality(self, theme_data: Dict[str, Any], 
        ...
        if not errors:
            ...
            details = f"{len(errors)}個の構造エラーが検出されました"
        
        report.add_quality_check("structure_compliance", passed, score, details)
    
    def _check_accessibility_quality(self, theme_data: Dict[str, Any], 
                                   report: QualityReport) -> None:
        """アクセシビリティ品質をチェックする"""
        self.logger.debug("アクセシビリティ品質チェックを実行します")
        
        start_time = time.time()
        accessibility_report = self.validation_service.validate_wcag_compliance(theme_data, "AA")
        validation_time = time.time() - start_time
        ...
        details = f"WCAG AA準拠率: {score:.1f}% ({accessibility_report.passed_checks}/{accessibility_report.total_checks})"
        
        report.add_quality_check("accessibility_compliance", passed, score, details)
    
    def _check_color_quality(self, theme_data: Dict[str, Any], 
                           report: QualityReport) -> None:
        """色品質をチェックする"""
        ...
        if not colors:
            report.add_quality_check("color_quality", False, 0.0, "色データが存在しません")
            return
        ...
        if missing_colors:
            ...
            passed = False
            details = f"必須色が不足しています: {', '.join(missing_colors)}"
        else:
            # コントラスト比チェック
            try:
                ...
                if contrast_ratio >= self.quality_thresholds['color_contrast_ratio']:
                    score = 100.0
                    passed = True
                    details = f"テキスト/背景のコントラスト比: {contrast_ratio:.2f}"
                else:
                    score = (contrast_ratio / self.quality_thresholds['color_contrast_ratio']) * 100.0
                    passed = False
                    details = f"コントラスト比が不十分です: {contrast_ratio:.2f} (最小: {self.quality_thresholds['color_contrast_ratio']})"
                    
            except Exception as e:
                score = 50.0
                passed = False
                details = f"コントラスト比計算エラー: {str(e)}"
        
        report.add_quality_check("color_quality", passed, score, details)
    
    def _check_performance_quality(self, theme_data: Dict[str, Any], 
                                 report: QualityReport) -> None:
        """パフォーマンス品質をチェックする"""
        ...
        if total_validation_time <= self.quality_thresholds['performance_validation_time']:
            time_score = 100.0
        else:
        ...
        details = f"データサイズ: {data_size/1024:.1f}KB, 検証時間: {total_validation_time:.3f}秒"
    ...
    def _check_required_properties(self, theme_data: Dict[str, Any], 
                                 report: QualityReport) -> None:
        """必須プロパティをチェックする"""
        ...
        for prop, expected_type in required_properties.items():
            if prop not in theme_data:
                ...
                missing_required.append(f"{prop} (型不正)")
        ...
        for prop, expected_type in optional_recommended_properties.items():
            if prop not in theme_data:
                ...
                missing_recommended.append(f"{prop} (型不正)")
        ...
        details_parts = []
        if missing_required:
            details_parts.append(f"必須プロパティ不足: {', '.join(missing_required)}")
        if missing_recommended:
            details_parts.append(f"推奨プロパティ不足: {', '.join(missing_recommended)}")
        
        details = "; ".join(details_parts) if details_parts else "すべての必須プロパティが存在します"
    ...
    def _generate_recommendations(self, report: QualityReport) -> None:
        ...
        if report.structure_errors:
            ...
            if any("必須フィールド" in error for error in report.structure_errors):
                report.add_recommendation("必須フィールド（name、version、colors、fonts）を追加してください")
        
        # アクセシビリティに基づく推奨事項
        if report.accessibility_report and not report.accessibility_report.is_compliant():
            report.add_recommendation("WCAG準拠のためにコントラスト比を改善してください")
            if report.accessibility_report.violations:
                error_violations = [v for v in report.accessibility_report.violations if v['severity'] == 'error']
                if error_violations:
                    report.add_recommendation("重要なアクセシビリティ違反を優先的に修正してください")
        ...
        if performance_check and not performance_check['passed']:
            ...
            if '検証時間' in performance_check['details']:
                report.add_recommendation("テーマ構造を簡素化して検証時間を短縮してください")
        ...
        if color_check and not color_check['passed']:
            if '必須色が不足' in color_check['details']:
                report.add_recommendation("必須色（background、text、primary）を定義してください")
            if 'コントラスト比が不十分' in color_check['details']:
                report.add_recommendation("テキストと背景のコントラスト比を4.5:1以上にしてください")
        
        # 総合スコアに基づく推奨事項
        if report.overall_score < 70.0:
            report.add_recommendation("品質スコアが低いため、全体的な見直しを推奨します")
        elif report.overall_score < 85.0:
            report.add_recommendation("品質向上のため、警告項目の修正を検討してください")
    
    def run_automated_test_suite(self, theme_data: Dict[str, Any], 
                                theme_name: str = None) -> Dict[str, Any]:
        """自動テストスイートを実行する
        ...
        try:
            ...
            self.logger.info(f"自動テスト完了: {theme_name} ({passed_tests}/{total_tests} 成功)")
            return test_results
            
        except Exception as e:
            self.logger.error(f"自動テスト中にエラーが発生しました: {str(e)}")
    ...
    def _test_basic_structure(self, theme_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'passed': len(errors) == 0,
                'message': '基本構造は正常です' if len(errors) == 0 else f'{len(errors)}個のエラーが検出されました',
                'errors': errors
            }
        except Exception as e:
            return {
                'passed': False,
                'message': f'構造テスト中にエラーが発生しました: {str(e)}',
                'errors': [str(e)]
            }
    ...
    def _test_color_data(self, theme_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'passed': True,
                'message': f'色データは正常です（{len(colors)}色定義済み）',
                'errors': []
            }
            ...
            return {
                'passed': False,
                'message': f'色データテスト中にエラーが発生しました: {str(e)}',
                'errors': [str(e)]
            }
    ...
    def _test_font_data(self, theme_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'passed': True,
                'message': f'フォントデータは正常です（{len(fonts)}フォント定義済み）',
                'errors': []
            }
            ...
            return {
                'passed': False,
                'message': f'フォントデータテスト中にエラーが発生しました: {str(e)}',
                'errors': [str(e)]
            }
    ...
    def _test_accessibility(self, theme_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'message': f'アクセシビリティスコア: {accessibility_report.score:.1f}%',
                'errors': [v['description'] for v in accessibility_report.violations if v['severity'] == 'error'],
                'warnings': [v['description'] for v in accessibility_report.violations if v['severity'] == 'warning'],
                'score': accessibility_report.score
            }
            ...
            return {
                'passed': False,
                'message': f'アクセシビリティテスト中にエラーが発生しました: {str(e)}',
                'errors': [str(e)]
            }
    ...
    def _test_json_serialization(self, theme_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'passed': False,
                'message': f'JSONシリアライゼーションテスト中にエラーが発生しました: {str(e)}',
                'errors': [str(e)]
            }
    
    def generate_ci_report(self, theme_data: Dict[str, Any], 
        ...
        ci_report = {
            ...
            'ci_summary': {
                'overall_status': 'PASS' if quality_report.overall_score >= 70.0 and test_results['summary']['success_rate'] >= 80.0 else 'FAIL',
        ...
        self.logger.info(f"CI/CDレポートを生成しました: {output_path}")
        return output_path