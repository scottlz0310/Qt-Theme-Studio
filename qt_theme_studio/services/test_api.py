...
class HeadlessTestRunner:
    ...
    def run_theme_loading_test(self, theme_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("テーマ読み込み失敗: {theme_path} - {str()}")
        ...
        try:
            ...
            for format_name in export_formats:
                ...
                try:
                    ...
                    format_result['error_message'] = str()
                    self.logger.warning("{format_name}エクスポート失敗: {str()}")
            ...
            result['error_message'] = str()
            self.logger.error("エクスポートテスト中にエラーが発生しました: {str()}")
    ...
    def run_theme_validation_test(self, theme_data: Dict[str, Any],
        ...
        try:
            ...
            result['error_message'] = str()
            self.logger.error("検証テスト中にエラーが発生しました: {str()}")
        ...
        try:
            ...
            for format_name in formats_to_test:
                ...
                try:
                    ...
                    format_result['error_message'] = str()
                    self.logger.warning("{format_name}ラウンドトリップ失敗: {str()}")
            ...
            result['error_message'] = str()
            self.logger.error("ラウンドトリップテスト中にエラーが発生しました: {str()}")
    ...
    def run_performance_test(self, theme_data: Dict[str, Any],
        ...
        try:
            ...
            for format_name in ['json', 'qss', 'css']:
                for i in range(iterations):
                    try:
                        ...
                        self.logger.warning(f"{format_name}エクスポートエラー (反復{i+1}): {str()}")
            ...
            result['error_message'] = str()
            self.logger.error("パフォーマンステスト中にエラーが発生しました: {str()}")
        ...
        try:
            ...
            for test_method, args in test_methods:
                try:
                    ...
                    self.logger.error(f"テスト実行エラー: {str()}")
            ...
            suite_result['error_message'] = str()
            self.logger.error("テストスイート実行中にエラーが発生しました: {str()}")
