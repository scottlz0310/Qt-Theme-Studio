...
class QtAdapter:
    ...
    def detect_qt_framework(self) -> str:
        ...
        for framework in frameworks:
            try:
                ...
                self.logger.debug(f"フレームワーク {framework} は利用できません")
                continue
    ...
    def get_qt_modules(self) -> Dict[str, Any]:
        ...
        try:
            if framework == 'PySide6':
                ...
                raise QtFrameworkNotFoundError(f"サポートされていないフレームワーク: {framework}")
            ...
            error_msg = f"{framework}のモジュール読み込みに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise QtFrameworkNotFoundError(error_msg)
    ...
    def create_application(self, app_name: str = "Qt-Theme-Studio") -> Any:
        ...
        try:
            ...
            error_msg = f"QApplicationの作成に失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise QtFrameworkNotFoundError(error_msg)
    ...
    def get_framework_info(self) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.debug(f"バージョン情報の取得に失敗しました: {str(e)}")
    ...
    def is_initialized(self) -> bool:
        ...
        Returns:
            bool: 初期化済みの場合True
        """
        return (self._detected_framework is not None and 
                self._qt_modules is not None)
