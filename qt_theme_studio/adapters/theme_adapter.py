...
class ThemeAdapter:
    ...
    def save_theme(self, theme_data: Dict[str, Any], path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            if isinstance(e, (ThemeSaveError, ThemeValidationError)):
                raise
            error_msg = f"テーマファイルの保存に失敗しました: {str(e)}"
            self.logger.error(error_msg)
    ...
    def _load_json_theme(self, theme_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"JSONファイルの解析に失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeLoadError(error_msg)
        except Exception as e:
            error_msg = f"JSONファイルの読み込みに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeLoadError(error_msg)
    ...
    def _load_qss_theme(self, theme_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"QSSファイルの読み込みに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeLoadError(error_msg)
    ...
    def _load_css_theme(self, theme_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"CSSファイルの読み込みに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeLoadError(error_msg)
    ...
    def _export_to_json(self, theme_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            error_msg = f"JSON形式でのエクスポートに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeExportError(error_msg)
    ...
    def _export_to_qss(self, theme_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            error_msg = f"QSS形式でのエクスポートに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeExportError(error_msg)
    ...
    def _export_to_css(self, theme_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            error_msg = f"CSS形式でのエクスポートに失敗しました: {str(e)}"
            self.logger.error(error_msg)
            raise ThemeExportError(error_msg)
    ...
    def _validate_theme_structure(self, theme_data: Dict[str, Any], result: Dict[str, Any]) -> None:
        """テーマデータの構造を検証する"""
        # バージョン情報の確認
    ...
    def _validate_theme_colors(self, theme_data: Dict[str, Any], result: Dict[str, Any]) -> None:
        """テーマの色設定を検証する"""
            ...
            if isinstance(colors, dict):
                for color_name, color_value in colors.items():
                    if not self._is_valid_color(color_value):
                        result['warnings'].append(f"無効な色値が検出されました: {color_name} = {color_value}")
    ...
    def _generate_qss_from_theme(self, theme_data: Dict[str, Any]) -> str:
        ...
        if 'colors' in theme_data and isinstance(theme_data['colors'], dict):
            ...
            if 'background' in colors:
                qss_lines.append(f"QWidget {{ background-color: {colors['background']}; }}")
            
            if 'text' in colors:
                qss_lines.append(f"QWidget {{ color: {colors['text']}; }}")
            
            if 'primary' in colors:
                qss_lines.append(f"QPushButton {{ background-color: {colors['primary']}; }}")
    ...
    def _generate_css_from_theme(self, theme_data: Dict[str, Any]) -> str:
        ...
        if 'colors' in theme_data and isinstance(theme_data['colors'], dict):
            ...
            # 基本的なスタイル
            if 'background' in colors:
                css_lines.append(f"body {{ background-color: var(--background); }}")
            
            if 'text' in colors:
                css_lines.append(f"body {{ color: var(--text); }}")
