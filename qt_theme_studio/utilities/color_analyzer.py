...
class ColorInfo:
    ...
    def __str__(self) -> str:
        return f"ColorInfo(hex={self.hex_value}, rgb={self.rgb}, luminance={self.luminance:.3f})"
...
class ColorAnalyzer:
    ...
    def hex_to_rgb(self, hex_color: str) -> Tuple[int, int, int]:
        ...
        if len(hex_color) != 6 or not re.match(r'^[0-9A-F]{6}$', hex_color):
            raise ValueError(f"無効な16進数カラーコード: #{hex_color}")
        ...
        try:
            ...
            raise ValueError(f"16進数カラーコードの変換に失敗しました: #{hex_color}") from e
    ...
    def calculate_contrast_ratio(self, color1: str, color2: str) -> float:
        ...
        try:
            ...
            logger.debug(f"コントラスト比計算: {color1} vs {color2} = {contrast_ratio:.2f}:1")
            
            return contrast_ratio
            
        except ValueError as e:
            logger.error(f"コントラスト比計算エラー: {e}")
            raise
    
    def get_wcag_compliance_level(self, contrast_ratio: float, is_large_text: bool = False) -> str:
        """
        WCAG適合レベルを判定
    ...
    def get_color_harmony(self, base_color: str, harmony_type: str = "complementary") -> List[str]:
        """
        調和色を生成
        ...
        try:
            ...
            if harmony_type == "complementary":
                # 補色 (180度回転)
                comp_h = (h + 180) % 360
                harmony_colors.append(self._hsl_to_hex(comp_h, s, l))
                
            elif harmony_type == "triadic":
                # 三色調和 (120度ずつ)
                for offset in [120, 240]:
                    new_h = (h + offset) % 360
                    harmony_colors.append(self._hsl_to_hex(new_h, s, l))
            ...
            logger.debug(f"調和色生成: {base_color} -> {harmony_colors}")
            return harmony_colors
            
        except ValueError as e:
            logger.error(f"調和色生成エラー: {e}")
            return []
    ...
    def analyze_color_accessibility(self, colors: Dict[str, Dict[str, str]]) -> AccessibilityReport:
        """
        色アクセシビリティを分析
            ...
            try:
                ...
                if wcag_level == "FAIL":
                    violation = AccessibilityViolation(
                        ...
                        suggestion=f"{element_name}のコントラスト比を{self.WCAG_AA_NORMAL:.1f}:1以上にしてください"
                    )
                    violations.append(violation)
                    
            except ValueError as e:
                logger.error(f"色分析エラー ({element_name}): {e}")
                continue
        ...
        score = (compliant_elements / total_elements * 100) if total_elements > 0 else 100
        ...
        logger.info(f"アクセシビリティ分析完了: スコア {score:.1f}%, レベル {wcag_level}")
        
        return report
    
    def get_minimum_contrast_colors(self, target_ratio: float = WCAG_AA_NORMAL) -> List[Tuple[str, str]]:
        """
        ...
        color_combinations.sort(
            key=lambda x: self.calculate_contrast_ratio(x[0], x[1]), 
            reverse=True
        )
