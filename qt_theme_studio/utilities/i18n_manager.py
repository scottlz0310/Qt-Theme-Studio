...
class I18nManager:
    ...
    def _initialize_translations(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"翻訳システムの初期化に失敗しました: {str(e)}", LogCategory.SYSTEM)
        ...
        if os.path.exists(translation_file):
            if self.translator.load(translation_file):
                ...
                if app:
                    ...
                    self.logger.info(f"翻訳ファイルを読み込みました: {translation_file}", LogCategory.SYSTEM)
                else:
                    self.logger.warning("QApplicationインスタンスが見つかりません", LogCategory.SYSTEM)
            else:
                self.logger.warning(f"翻訳ファイルの読み込みに失敗しました: {translation_file}", LogCategory.SYSTEM)
        else:
            self.logger.debug(f"翻訳ファイルが見つかりません: {translation_file}", LogCategory.SYSTEM)
    ...
    def _setup_qt_translator(self) -> None:
        ...
        qt_translation_file = f"qt_{self.current_language}"
        
        if self.qt_translator.load(qt_translation_file, qt_translation_path):
            app = self.QtWidgets.QApplication.instance()
            if app:
                app.installTranslator(self.qt_translator)
                self.logger.info(f"Qt標準翻訳を読み込みました: {qt_translation_file}", LogCategory.SYSTEM)
        else:
            self.logger.debug(f"Qt標準翻訳の読み込みに失敗しました: {qt_translation_file}", LogCategory.SYSTEM)
    ...
    def set_language(self, language: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"言語を変更しました: {language}", LogCategory.SYSTEM)
            return True
            
        except Exception as e:
            self.logger.error(f"言語の変更に失敗しました: {str(e)}", LogCategory.SYSTEM)
            return False
    ...
    def create_translation_file(self) -> str:
        ...
        try:
            ...
            self.logger.info(f"翻訳ファイルを作成しました: {ts_file}", LogCategory.SYSTEM)
            return ts_file
            
        except Exception as e:
            self.logger.error(f"翻訳ファイルの作成に失敗しました: {str(e)}", LogCategory.SYSTEM)
            return ""
...
'''
    ...
    def handle_japanese_file_paths(self, file_path: str) -> str:
        ...
        try:
            ...
            self.logger.error(f"日本語ファイルパスの処理に失敗しました: {str(e)}", LogCategory.SYSTEM)
            # フォールバック: ASCII文字のみのパスを生成
            import tempfile
            temp_dir = tempfile.gettempdir()
            fallback_path = os.path.join(temp_dir, "qt_theme_studio_temp")
            self.logger.warning(f"フォールバックパスを使用します: {fallback_path}", LogCategory.SYSTEM)
    ...
    def get_localized_error_message(self, error_key: str, **kwargs) -> str:
        ...
        try:
            ...
            self.logger.warning(f"エラーメッセージのフォーマットに失敗しました: {str(e)}", LogCategory.SYSTEM)
    ...
    def get_localized_status_message(self, status_key: str, **kwargs) -> str:
        ...
        try:
            ...
            self.logger.warning(f"ステータスメッセージのフォーマットに失敗しました: {str(e)}", LogCategory.SYSTEM)
            return message_template