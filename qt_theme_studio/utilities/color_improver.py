...
class ColorImprover:
    ...
    def improve_contrast(
        ...
        """
        try:
            current_ratio = self.color_analyzer.calculate_contrast_ratio(
                foreground, background
            )

            if current_ratio >= target_ratio:
                logger.debug("コントラスト比は既に十分です: {current_ratio:.2f}:1")
            ...
            if improved_fg:
                new_ratio = self.color_analyzer.calculate_contrast_ratio(
                    improved_fg, background
                )
                if new_ratio >= target_ratio:
                    logger.info(
                        "前景色調整でコントラスト改善: {current_ratio:.2f} -> {new_ratio:.2f}"
                    )
            ...
            if improved_bg:
                new_ratio = self.color_analyzer.calculate_contrast_ratio(
                    foreground, improved_bg
                )
                if new_ratio >= target_ratio:
                    logger.info(
                        "背景色調整でコントラスト改善: {current_ratio:.2f} -> {new_ratio:.2f}"
                    )
            ...
            if improved_fg and improved_bg:
                new_ratio = self.color_analyzer.calculate_contrast_ratio(
                    improved_fg, improved_bg
                )
                if new_ratio >= target_ratio:
                    logger.info(
                        "両色調整でコントラスト改善: {current_ratio:.2f} -> {new_ratio:.2f}"
                    )
                    return improved_fg, improved_bg

            # 最後の手段：極端な色に変更
            logger.warning(
                "標準的な調整では目標コントラスト比に達しませんでした。極端な色を使用します。"
            )
            return self._get_extreme_contrast_colors(target_ratio)

        except Exception:
            logger.error("コントラスト改善エラー: {e}")
            return foreground, background
    ...
    def _adjust_color_for_contrast(
        ...
        try:
            ...
            h, s, lightness = self.color_analyzer.rgb_to_hsl(*rgb)
            ...
            for i in range(1, max_iterations + 1):
                ...
                if adjust_foreground:
                    ratio = self.color_analyzer.calculate_contrast_ratio(
                        new_color, reference_color
                    )
                else:
                    ratio = self.color_analyzer.calculate_contrast_ratio(
                        reference_color, new_color
                    )
            ...
            logger.error("色調整エラー: {e}")
            return None

    def _hsl_to_hex(self, h: float, s: float, l: float) -> str:
        """HSL値を16進数カラーコードに変換"""
        return self.color_analyzer._hsl_to_hex(h, s, lightness)
    ...
    def suggest_accessible_alternatives(
        ...
        for violation in report.violations:
            ...
            improved_fg, improved_bg = self.improve_contrast(
                violation.foreground_color, violation.background_color, target_ratio
            )

            # 改善後のコントラスト比を計算
            improved_ratio = self.color_analyzer.calculate_contrast_ratio(
                improved_fg, improved_bg
            )
        ...
        logger.info("{len(improvements)}個の色改善提案を生成しました")
        return improvements
    ...
    def _generate_improvement_description(
        ...
        """改善説明文を生成"""
        base_desc = "{element_name}のコントラスト比を{original_ratio:.1f}:1から{improved_ratio:.1f}:1に改善"
        ...
        return "{base_desc}{type_desc}し、{level_desc}しました。"

    def get_accessibility_preset(
        self, preset_name: str
    ) -> Optional[AccessibilityPreset]:
        """アクセシビリティプリセットを取得"""
    ...
    def apply_accessibility_preset(
        self,
        preset_name: str,
        existing_colors: Optional[Dict[str, Dict[str, str]]] = None,
        ...
        logger.info("プリセット '{preset.name}' を適用しました")
        return result_colors

    def generate_color_variations(
        self, base_colors: Dict[str, Dict[str, str]], variation_type: str = "lightness"
        ...
        logger.info("{len(compliant_variations)}個の適合バリエーションを生成しました")
        return compliant_variations

    def _adjust_colors_lightness(
        self, colors: Dict[str, Dict[str, str]], adjustment: float
    ) -> Dict[str, Dict[str, str]]:
        """色の明度を調整"""
        adjusted_colors = {}
        ...
        for element_name, color_pair in colors.items():
            ...
            for color_type, hex_color in color_pair.items():
                try:
                    ...
                    logger.warning("色調整エラー ({element_name}, {color_type}): {e}")
        ...
        for element_name, color_pair in colors.items():
            ...
            for color_type, hex_color in color_pair.items():
                try:
                    ...
                    adjusted_color = self._hsl_to_hex(h, new_s, lightness)
                    adjusted_pair[color_type] = adjusted_color
        ...
        for element_name, color_pair in colors.items():
            ...
            for color_type, hex_color in color_pair.items():
                try:
                    ...
                    adjusted_color = self._hsl_to_hex(new_h, s, lightness)
                    adjusted_pair[color_type] = adjusted_color
    ...
    def validate_color_scheme(
        ...
        if report.violations:
            issues.extend(
                [
                    "{v.element_name}: コントラスト比不足 ({v.contrast_ratio:.1f}:1)"
        ...
        logger.info(
            "色スキーム検証完了: {'妥当' if is_valid else '問題あり'} ({len(issues)}個の問題)"
        )
