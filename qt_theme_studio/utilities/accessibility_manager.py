...
class AccessibilityManager:
    ...
    def _get_focusable_widgets(self, parent) -> List[Any]:
        ...
        def collect_widgets(widget):
            if hasattr(widget, "focusPolicy"):
                ...
                if policy in [
                    self.QtCore.Qt.FocusPolicy.TabFocus,
    ...
    def _update_screen_reader_info(self, widget) -> None:
        ...
        if hasattr(widget, "setAccessibleName") and not widget.accessibleName():
            ...
            if hasattr(widget, "text") and widget.text():
                ...
                widget.setAccessibleName("{widget_type}ウィジェット")
    ...
    def _set_widget_accessibility_attributes(self, parent) -> None:
        ...
        def set_attributes(widget):
            ...
            if widget_type in ["QPushButton", "QToolButton"]:
                if hasattr(widget, "setAccessibleDescription") and hasattr(
                    widget, "toolTip"
                ):
    ...
    def check_color_contrast(
        ...
        try:
            ...
            # WCAG基準値を取得
            aa_threshold = (
                self.wcag_large_text_aa_ratio if is_large_text else self.wcag_aa_ratio
            )
            aaa_threshold = (
                self.wcag_large_text_aaa_ratio if is_large_text else self.wcag_aaa_ratio
            )
            ...
            if not aa_compliant:
                result["recommendations"].append(
                    "コントラスト比が不十分です。{aa_threshold:.1f}:1以上にしてください。"
                )
            elif not aaa_compliant:
                result["recommendations"].append(
                    "AAA準拠にするには、コントラスト比を{aaa_threshold:.1f}:1以上にしてください。"
                )
            else:
            ...
            self.logger.error(
                f"コントラスト比チェックに失敗しました: {str(e)}", LogCategory.SYSTEM
            )
            return {
                "contrast_ratio": 0.0,
                "aa_compliant": False,
        ...
        try:
            ...
            for fg_key, bg_key in color_combinations:
                if fg_key in colors and bg_key in colors:
                    ...
                    contrast_ratios["{fg_key}_{bg_key}"] = contrast_result[
                        "contrast_ratio"
                    ]

                    if not contrast_result["aa_compliant"]:
                        violations.append(
                            "{fg_key}と{bg_key}のコントラスト比が不十分です "
                            "({contrast_result['contrast_ratio']:.2f}:1)"
                        )
                        suggestions.append(
                            "{fg_key}と{bg_key}の色を調整してコントラスト比を改善してください"
                        )
            ...
            for font_key, font_data in fonts.items():
                if isinstance(font_data, dict) and "size" in font_data:
                    ...
                    if font_size < 12:
                        violations.append(
                            "{font_key}のフォントサイズが小さすぎます ({font_size}pt)"
                        )
                        suggestions.append(
                            "{font_key}のフォントサイズを12pt以上にしてください"
                        )
