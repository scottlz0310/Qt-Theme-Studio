...
class ThemePropertyChangeCommand:
    ...
    def __init__(
        ...
        self.description = description or "プロパティ変更: {property_path}"
...
class ThemeController:
    ...
    def create_new_theme(self, theme_name: str = "新しいテーマ") -> Dict[str, Any]:
        ...
        try:
            ...
            new_theme = {
                ...
                "fonts": {
                    "default": {"family": "Arial", "size": 10, "weight": "normal"},
    ...
    def load_theme(self, theme_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.log_theme_operation(
                "テーマ読み込み", theme_name, file_path=str(theme_path)
            )
            self.logger.log_user_action(
                "テーマ読み込み",
                {"theme_name": theme_name, "file_path": str(theme_path)},
            )
    ...
    def save_theme(self, theme_path: Optional[Union[str, Path]] = None) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.log_theme_operation(
                    "テーマ保存", theme_name, file_path=str(save_path)
                )
                self.logger.log_user_action(
                    "テーマ保存",
                    {"theme_name": theme_name, "file_path": str(save_path)},
                )
    ...
    def change_theme_property(
        self, property_path: str, new_value: Any, description: str = ""
    ) -> bool:
    ...
    def undo_last_action(self) -> bool:
        ...
        try:
            if not self._undo_stack or not self._undo_stack.canUndo():
                self.logger.info(
                    "取り消し可能な操作がありません", LogCategory.USER_ACTION
                )
                return False
            ...
            self.logger.log_user_action(
                "Undo操作実行",
                {"undo_text": undo_text, "can_redo": self._undo_stack.canRedo()},
            )
    ...
    def redo_last_action(self) -> bool:
        ...
        try:
            if not self._undo_stack or not self._undo_stack.canRedo():
                self.logger.info(
                    "再実行可能な操作がありません", LogCategory.USER_ACTION
                )
                return False
            ...
            self.logger.log_user_action(
                "Redo操作実行",
                {"redo_text": redo_text, "can_undo": self._undo_stack.canUndo()},
            )
    ...
    def _on_undo_stack_changed(self, index: int) -> None:
        ...
        try:
            ...
            self.logger.debug(
                "UndoStackインデックスが変更されました: {index}", LogCategory.SYSTEM
            )
