...
class ColorPicker:
    ...
    def _open_color_dialog(self) -> None:
        ...
        color = self.QtWidgets.QColorDialog.getColor(
            self.current_color, self.parent, "色を選択"
        )
    ...
    def _notify_color_changed(self) -> None:
        ...
        if self.color_changed_callback:
            self.color_changed_callback(self.current_color.name(), self.current_style)
    ...
    def set_color_changed_callback(self, callback: Callable[[str, str], None]) -> None:
        """色変更コールバックを設定します
...
class ThemeEditor:
    ...
    def _on_color_changed(self, color: str, style: str) -> None:
        ...
        if hasattr(self, "main_window_reference") and hasattr(
            self.main_window_reference, "current_theme_data"
        ):
        ...
        self.logger.debug(
            "色が変更されました: {color}, スタイル: {style}", LogCategory.UI
        )
    ...
    def load_theme(self, theme_data: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            "テーマを読み込みました: {theme_data.get('name', '無名')}", LogCategory.UI
        )
    ...
    def set_theme_changed_callback(
    ...
    def set_color_property(self, property_name: str, color_value: str) -> None:
        ...
        self.logger.debug(
            "色プロパティを設定しました: {property_name}={color_value}", LogCategory.UI
        )
    ...
    def connect_to_preview_window(self, preview_window) -> None:
        ...
        if hasattr(preview_window, "update_preview"):
            ...
            self.logger.warning(
                "プレビューウィンドウにupdate_previewメソッドがありません",
                LogCategory.UI,
            )
    ...
    def disconnect_from_preview_window(self, preview_window) -> None:
        ...
        if hasattr(preview_window, "update_preview"):
            ...
            self.logger.info(
                "プレビューウィンドウとの連携を解除しました", LogCategory.UI
            )
        ...
        try:
            ...
            if not file_path:
                file_path, _ = self.QtWidgets.QFileDialog.getSaveFileName(
                    self.widget,
                    "テーマを保存",
                    "{self.current_theme.get('name', '新しいテーマ')}.json",
    ...
    def _show_save_success_message(self, file_path: str) -> None:
        ...
        msg_box.setText(
            "テーマ '{self.current_theme.get('name', '無名')}' を正常に保存しました。"
        )
        msg_box.setDetailedText("保存場所: {file_path}")
        msg_box.setInformativeText("ファイル名: {file_name}")
        ...
        self.logger.info(
            "保存成功メッセージを表示しました: {file_name}", LogCategory.UI
        )
    ...
    def _show_save_error_message(self, error_message: str) -> None:
        ...
        self.logger.error(
            "保存エラーメッセージを表示しました: {error_message}", LogCategory.UI
        )
    ...
    def export_theme(self, format: str = "json", file_path: str = None) -> bool:
        ...
        try:
            ...
            if format not in extensions:
                raise ValueError("サポートされていないエクスポート形式: {format}")

            ext, file_filter = extensions[format]

            # ファイルパスが指定されていない場合はダイアログを表示
            if not file_path:
                default_name = "{self.current_theme.get('name', '新しいテーマ')}.{ext}"
                file_path, _ = self.QtWidgets.QFileDialog.getSaveFileName(
                    self.widget,
                    "テーマを{format.upper()}形式でエクスポート",
                    default_name,
            ...
            self.logger.info(
                "テーマを{format.upper()}形式でエクスポートしました: {file_path}",
                LogCategory.UI,
            )
            return True
    ...
    def _show_export_success_message(self, file_path: str, format: str) -> None:
        ...
        msg_box.setIcon(self.QtWidgets.QMessageBox.Icon.Information)
        msg_box.setText("テーマを{format.upper()}形式で正常にエクスポートしました。")
        msg_box.setDetailedText("保存場所: {file_path}")
        msg_box.setInformativeText("ファイル名: {file_name}")
        ...
        self.logger.info(
            "エクスポート成功メッセージを表示しました: {file_name}", LogCategory.UI
        )
    ...
    def load_theme_from_file(self, file_path: str = None) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "テーマファイルを読み込みました: {file_path}", LogCategory.UI
            )
            return True
    ...
    def _show_load_success_message(self, file_path: str) -> None:
        ...
        msg_box.setIcon(self.QtWidgets.QMessageBox.Icon.Information)
        msg_box.setText(
            "テーマ '{self.current_theme.get('name', '無名')}' を正常に読み込みました。"
        )
        msg_box.setDetailedText("読み込み元: {file_path}")
        msg_box.setInformativeText("ファイル名: {file_name}")
        ...
        self.logger.info(
            "読み込み成功メッセージを表示しました: {file_name}", LogCategory.UI
        )
    ...
    def _show_load_error_message(self, error_message: str) -> None:
        ...
        self.logger.error(
            "読み込みエラーメッセージを表示しました: {error_message}", LogCategory.UI
        )
