  ...
  qt-theme-studio export theme.json --format qss --output theme.qss
    ...
    def load_theme_file(self, theme_file: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(theme_file):
            raise CLIError(f"テーマファイルが存在しません: {theme_file}")
        
        try:
            return self.theme_service.load_theme_from_file(theme_file)
        except Exception as e:
            raise CLIError(f"テーマファイル読み込みエラー: {str(e)}")
    ...
    def cmd_quality_check(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(f"テーマファイル: {args.theme_file}")
            print(f"品質閾値: {args.threshold}")
            print()
            ...
            # 品質チェック実行
            report = self.quality_service.run_comprehensive_quality_check(theme_data, theme_name)
            
            # 結果表示
            print("=" * 60)
            print("品質チェック結果")
            print("=" * 60)
            print(f"テーマ名: {report.theme_name}")
            print(f"総合スコア: {report.overall_score:.1f}/100")
            print(f"品質判定: {'合格' if report.overall_score >= args.threshold else '不合格'}")
            print()
            ...
            if report.quality_checks:
                ...
                for check_name, check_data in report.quality_checks.items():
                    ...
                    print(f"  {status} {check_name}: {check_data['score']:.1f}/100")
            ...
            if report.recommendations:
                print(f"推奨事項 ({len(report.recommendations)}):")
            ...
            if args.output:
                ...
                print(f"詳細レポートを {args.output} に出力しました")
    ...
    def cmd_test(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(f"テーマファイル: {args.theme_file}")
            print(f"パフォーマンステスト反復回数: {args.iterations}")
            print()
            
            # 包括的テストスイート実行
            result = self.test_runner.run_comprehensive_test_suite(theme_path=args.theme_file)
            ...
            # 結果ファイル出力
            output_file = args.output or f"test_results_{int(result['start_time'])}.json"
    ...
    def cmd_ci_report(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(f"テスト成功率閾値: {args.test_threshold}")
            print()
            ...
            # CI/CDレポート生成
            report_path = self.quality_service.generate_ci_report(theme_data, args.output)
            ...
            print("CI/CD統合レポート")
            print("=" * 60)
            print(f"総合判定: {ci_summary['overall_status']}")
            print(f"品質スコア: {ci_summary['quality_score']:.1f}/100")
            print(f"テスト成功率: {ci_summary['test_success_rate']:.1f}%")
            print()
            
            if ci_summary['recommendations']:
                print(f"推奨事項 ({len(ci_summary['recommendations'])}):")
    ...
    def cmd_validate(self, args: argparse.Namespace) -> int:
        ...
        try:
            print("テーマ検証を開始します...")
            print(f"テーマファイル: {args.theme_file}")
            print(f"WCAGレベル: {args.wcag_level}")
            print()
            ...
            # 構造検証
            structure_errors = self.quality_service.validation_service.validate_theme_structure(theme_data)
            
            # アクセシビリティ検証
            accessibility_report = self.quality_service.validation_service.validate_wcag_compliance(
            ...
            # 構造検証結果
            if structure_errors:
                print(f"構造エラー ({len(structure_errors)}):")
            ...
            # アクセシビリティ検証結果
            print(f"アクセシビリティ検証 (WCAG {args.wcag_level}):")
            print(f"  スコア: {accessibility_report.score:.1f}%")
            print(f"  準拠状況: {'準拠' if accessibility_report.is_compliant() else '非準拠'}")
            
            if accessibility_report.violations:
                print(f"  違反 ({len(accessibility_report.violations)}):")
            ...
            if args.output:
                validation_result = {
                    ...
                    'overall_valid': len(structure_errors) == 0 and accessibility_report.is_compliant()
                }
                
                with open(args.output, 'w', encoding='utf-8') as f:
                    json.dump(validation_result, f, ensure_ascii=False, indent=2)
                
                print(f"検証結果を {args.output} に出力しました")
            
            # 終了コード決定
            is_valid = len(structure_errors) == 0 and accessibility_report.is_compliant()
    ...
    def cmd_export(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            # エクスポート実行
            export_result = self.test_runner.run_theme_export_test(theme_data, [args.format])
            ...
            if export_result['success']:
                ...
                if format_result['success']:
                    # エクスポートデータを取得して保存
                    if args.format == 'json':
                        exported_data = self.quality_service.export_service.export_theme(theme_data, 'json')
                    elif args.format == 'qss':
                        exported_data = self.quality_service.export_service.export_theme(theme_data, 'qss')
                    elif args.format == 'css':
                        exported_data = self.quality_service.export_service.export_theme(theme_data, 'css')
                    
                    with open(args.output, 'w', encoding='utf-8') as f:
                        f.write(exported_data)
                    
                    print("✓ エクスポート成功")
                    print(f"出力サイズ: {format_result['output_size']} バイト")
                    print(f"処理時間: {format_result['export_time']:.3f}秒")
                    return 0
                else:
                    print(f"✗ エクスポート失敗: {format_result.get('error_message', '不明なエラー')}")
                    return 1
            else:
                print(f"✗ エクスポート失敗: {export_result.get('error_message', '不明なエラー')}")
                return 1
