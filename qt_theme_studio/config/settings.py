...
class ApplicationSettings:
    ...
    def load_settings(self) -> Dict[str, Any]:
        ...
        try:
            if self.settings_file.exists():
                ...
                # デフォルト設定とマージ
                self._settings = self._merge_settings(get_default_settings(), loaded_settings)
    ...
    def save_settings(self, settings: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(f"設定ファイルを保存しました: {self.settings_file}")
    ...
    def get_recent_themes(self) -> List[str]:
        ...
        for theme_path in recent_themes:
            if os.path.exists(theme_path):
                ...
                self.logger.debug(f"存在しないテーマファイルを除外しました: {theme_path}")
    ...
    def add_recent_theme(self, theme_path: str) -> None:
        ...
        self.logger.info(f"最近使用したテーマリストに追加しました: {abs_path}")
    ...
    def remove_recent_theme(self, theme_path: str) -> None:
        ...
        if abs_path in recent_themes:
            ...
            self._settings['theme']['recent_themes'] = recent_themes
            self.save_settings(self._settings)
            self.logger.info(f"最近使用したテーマリストから削除しました: {abs_path}")
