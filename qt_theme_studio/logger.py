...
class JapaneseFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # 基本フォーマット
        formatted_message = (
            "[{timestamp}] [{japanese_level}] [{category}] {record.getMessage()}"
        )

        # スレッド情報を含める場合
        if self.include_thread and hasattr(record, "thread"):
            formatted_message += " (スレッド: {record.thread})"

        # モジュール情報を追加
        if hasattr(record, "module") and record.module:
            formatted_message += " ({record.module})"
...
class Logger:
    ...
    def __init__(
        self, name: str = "qt_theme_studio", log_directory: Optional[str] = None
    ):
        """
        ロガーを初期化します
    ...
    def log_error(
        ...
        Args:
            ...
            **kwargs: 追加情報
        """
        extra = {"category": LogCategory.ERROR.value, **kwargs}
    ...
    def log_user_action(
        ...
        # ログ出力
        message = "ユーザー操作: {action}"
        if details:
            details_str = ", ".join(["{k}={v}" for k, v in details.items()])
    ...
    def start_performance_timer(self, operation: str) -> None:
        ...
        self.debug("パフォーマンス測定開始: {operation}", LogCategory.PERFORMANCE)
    ...
    def log_performance(
        ...
        message = "パフォーマンス: {operation} - {duration:.3f}秒 ({performance_level})"
    ...
    def log_theme_operation(
        self, operation: str, theme_name: Optional[str] = None, **kwargs
    ) -> None:
        """
        テーマ操作ログを記録します
        ...
        Args:
            ...
            **kwargs: 追加情報
        """
        message = "テーマ操作: {operation}"
        if theme_name:
            message += f" (テーマ: {theme_name})"

        extra = {"operation": operation, "theme_name": theme_name, **kwargs}

        self._log(logging.INFO, message, LogCategory.THEME, **extra)

    def log_file_operation(
        self, operation: str, file_path: str, success: bool = True, **kwargs
    ) -> None:
        """
        ファイル操作ログを記録します
    ...
    def log_ui_event(self, event: str, widget: Optional[str] = None, **kwargs) -> None:
        ...
        Args:
            ...
            **kwargs: 追加情報
        """
        message = "UIイベント: {event}"
        if widget:
            message += f" (ウィジェット: {widget})"

        extra = {"event": event, "widget": widget, **kwargs}
    ...
    def _log(self, level: int, message: str, category: LogCategory, **kwargs) -> None:
        ...
        Args:
            ...
            **kwargs: 追加情報
        """
        extra = {"category": category.value, **kwargs}
    ...
    def export_logs(
        ...
        try:
            ...
            self.info("ログをエクスポートしました: {output_file}", LogCategory.SYSTEM)
            return True
    ...
    def cleanup_old_logs(self, days_to_keep: int = 30) -> None:
        ...
        try:
            ...
            if cleaned_files:
                self.info(
                    "古いログファイルを削除しました: {len(cleaned_files)}個",
                    LogCategory.SYSTEM,
                )
...
def setup_logging(
    log_directory: Optional[str] = None, log_level: str = "INFO"
) -> Logger:
    """
    ログシステムを初期化します
