...
class ErrorHandler:
    ...
    def __init__(self, logger=None):
        ...
        self.crash_reports_directory = Path.home() / ".qt_theme_studio" / "crash_reports"
    ...
    def handle_qt_framework_error(self, error: QtFrameworkNotFoundError) -> Dict[str, Any]:
        """
        Qtフレームワークエラーを処理します
        ...
        if self.logger:
            self.logger.log_error(
                f"Qtフレームワークが検出されませんでした: {error.message}",
                error
            )
    ...
    def handle_theme_load_error(self, error: ThemeLoadError) -> Dict[str, Any]:
        ...
        if self.logger:
            self.logger.log_error(
                f"テーマの読み込みに失敗しました: {error.message}",
                error
            )
                ...
                recovery_actions.extend([
                    {
                        ...
                        "description": f"バックアップから復元 ({len(backup_files)}個のバックアップが利用可能)",
    ...
    def handle_application_crash(self, error: ApplicationCrashError) -> Dict[str, Any]:
        ...
        if self.logger:
            self.logger.log_error(
                f"アプリケーションがクラッシュしました: {error.message}",
                error
            )
    ...
    def _suggest_alternative_save_paths(self, original_path: str) -> list:
        ...
        # ホームディレクトリ
        home_path = Path.home() / "Documents" / "Qt-Theme-Studio" / original_path_obj.name
    ...
    def _generate_crash_report(self, error: ApplicationCrashError) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.logger:
                self.logger.log_error(f"クラッシュレポートの生成に失敗しました: {str(e)}")
    ...
    def _attempt_auto_save(self, crash_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "current_theme" in crash_context:
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                auto_save_file = self.backup_directory / f"auto_save_theme_{timestamp}.json"
            ...
            if "window_state" in crash_context:
                ...
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                state_file = self.backup_directory / f"auto_save_window_state_{timestamp}.json"
            ...
            return {
                ...
                "message": f"{len(saved_files)}個のファイルを自動保存しました"
            }
    ...
    def create_backup(self, file_path: str, data: Any) -> bool:
        ...
        try:
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            backup_file = self.backup_directory / f"{file_name}.{timestamp}.backup"
            ...
            if self.logger:
                self.logger.log_error(f"バックアップの作成に失敗しました: {str(e)}")
            return False